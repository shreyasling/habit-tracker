...
import WeeklyView from './components/Views/WeeklyView'; // Added import
...
function App() {
  ...
  // ... (rest of the state logic is same)
  ...
  // Check if month setup is needed when month changes
  // Month setup effect removed as per user request


  // Calculate stats for AI context
  const monthStats = useMemo(() => {
    // Filter tasks to only count days they apply to
    let totalCells = 0;
    ...
    for (let i = daysInMonth; i >= 1; i--) {
      ...
      for (const task of tasks) {
        const taskDays = task.days || days;
        if (!taskDays.includes(i)) continue; // Skip if task doesn't apply to this day
        if (!completionData[task.id]?.[dateKey]) {
          allCompleted = false;
          break;
        }
  ...
  useEffect(() => {
    ...
    // Only celebrate if streak increased by exactly 1 (user action) 
    // AND we hit a milestone.
    // This prevents celebrations when switching months (0 -> N jump)
  ...
  const handleToggleCompletion = useCallback(async (taskId, day, targetMonth = month, targetYear = year) => {
    ...
    // Check if task applies to this day - only if in current view (month)
    if (targetMonth === month && targetYear === year) {
      const task = tasks.find(t => t.id === taskId);
      if (task && task.days && !task.days.includes(day)) {
        return; // Task doesn't apply to this day
      }
    }
  ...
  // Add a new task (Firebase) - now month-specific with day selection
  const handleAddTask = useCallback(async (name, selectedDays = null) => {
    ...
    const newTask = {
      ...
      name,
      days: selectedDays || Array.from({ length: daysInMonth }, (_, i) => i + 1), // Use provided days or all days
